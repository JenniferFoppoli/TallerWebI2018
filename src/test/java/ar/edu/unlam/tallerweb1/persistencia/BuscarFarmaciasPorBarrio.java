...
public class BuscarFarmaciasPorBarrio  extends SpringTest{
	@Test
    ...
    public void buscarFarmacia(){
		...
		List<Farmacia> Resultado = getSession().createCriteria(Farmacia.class).createAlias("direccion.barrio","barrioBuscado").add(Restrictions.eq("barrioBuscado.nombre", "barrio2")).list();
    	for (Farmacia busqueda: Resultado){
    		assertThat(busqueda.getDireccion().getBarrio().getNombre()).isEqualTo("barrio2");
